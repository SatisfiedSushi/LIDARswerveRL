...
class OkayPlan():
    ...
    def _get_Obs_Penalty(self):
        ...
        # Reshape to (G, N, S) then sum over segments to get (G, N)
        Obs_penalty = Intersect_Matrix.sum(dim=1).reshape((self.G, self.N, self.S)).sum(dim=2) * self.params[
            10]  # (G, N)
        ...
        if self.Flat_pdct_segments is None or self.Flat_pdct_segments.numel() == 0:
            ...
            Pdct_Intersect_Matrix = self._Is_Seg_Ingersection_PtoM(flatted_Segments,
                                                                   self.Flat_pdct_segments)  # (P, M_pdct)
            # Sum over dynamic obstacle segments (dim=1) to get total intersections per segment
            Pdct_Obs_penalty = Pdct_Intersect_Matrix.sum(dim=1).reshape((self.G, self.N, self.S)).sum(dim=2) * \
                               self.params[50]  # (G, N)
    ...
    def _get_fitness(self):
        ...
        # print(f"Path_length shape: {Path_length.shape}")  # Should be [G, N]
        # print(f"Obs_penalty shape: {self.Obs_penalty.shape}")  # Should be [G, N]
        # print(f"Pdct_Obs_penalty shape: {Pdct_Obs_penalty.shape}")  # Should be [G, N]
    ...
    def plan(self, env_info):
        ...
        self.Obs_Segments = env_info['Obs_Segments'].to(self.dvc) if env_info[
                                                                         'Obs_Segments'].numel() > 0 else torch.empty(
            (0, 2, 2), dtype=torch.float32, device=self.dvc)
        self.Flat_pdct_segments = env_info['Flat_pdct_segments'].to(self.dvc) if env_info[
                                                                                     'Flat_pdct_segments'].numel() > 0 else torch.empty(
            (0, 2, 2), dtype=torch.float32, device=self.dvc)
