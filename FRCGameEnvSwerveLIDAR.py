...
class env(gym.Env):
    ...
    def __init__(self, render_mode="human"):
        ...
        self.observation_space = Dict({
            'Position': Box(low=np.array([-np.inf, -np.inf]), high=np.array([np.inf, np.inf]), dtype=np.float32),
            'Angle': Box(low=-np.pi, high=np.pi, shape=(1,), dtype=np.float32),
            'Velocity': Box(low=np.array([-np.inf, -np.inf]), high=np.array([np.inf, np.inf]), dtype=np.float32),
            'Angular Velocity': Box(low=-np.inf, high=np.inf, shape=(1,), dtype=np.float32),
            'LIDAR Distances': Box(low=0, high=np.inf, shape=(self.number_of_rays,), dtype=np.float32),
            'LIDAR Angles': Box(low=-np.pi, high=np.pi, shape=(self.number_of_rays,), dtype=np.float32),
        })
        self.action_space = Dict({
            'Velocity': Box(low=np.array([-1, -1]), high=np.array([1, 1]), shape=(2,)),
            'Angular Velocity': Box(low=np.array([-1]), high=np.array([1]), shape=(1,)),
        })
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.ray_distances, self.end_rays, self.ray_angles, self.distance_endpoints, self.raycast_points = self.LIDAR.cast_rays(self.robots[0], self.swerve_instances[
            0].get_box2d_instance().position, self.swerve_instances[0].get_box2d_instance().angle, 1, 1, 1, 4)
        ...
        obs = {
            ...
            'Velocity': np.array([0, 0]),
            'Angular Velocity': 0,
            'LIDAR Distances': np.array([0] * self.number_of_rays),
            'LIDAR Angles': np.array([0] * self.number_of_rays),
        }
    ...
    def step(self, actions):  # TODO: change action dictionary
        ...
        self.ray_distances, self.end_rays, self.ray_angles, self.distance_endpoints, self.raycast_points = self.LIDAR.cast_rays(self.robots[0],
                                                                                  self.robots[0].position,
                                                                                  self.swerve_instances[
                                                                                      0].get_box2d_instance().angle, 1,
                                                                                  1, 4, 4)
        ...
        # reward for one swerve based off of lidar distances
        rewards = 10 if np.average(self.ray_distances) == 0 else 1 / np.average(self.ray_distances)
        ...
        obs = {
            'Position': np.array([self.swerve_instances[0].get_box2d_instance().position.x, self.swerve_instances[0].get_box2d_instance().position.y]),
            'Angle': self.swerve_instances[0].get_box2d_instance().angle,
            'Velocity': np.array([self.swerve_instances[0].get_velocity()[0], self.swerve_instances[0].get_velocity()[1]]),
            'Angular Velocity': self.swerve_instances[0].get_angular_velocity(),
            'LIDAR Distances': np.array(self.ray_distances),
            'LIDAR Angles': np.array(self.ray_angles),
        }
    ...
    def render(self):
        ...
        # render LIDAR rays
        '''for lidar in self.b2LIDARs:
            for fixture in lidar.fixtures:
                fixture.shape.draw(lidar, fixture)'''
        for agent in self.agents:
            swerve = self.swerve_instances[self.agents.index(agent)]
            for fixture in swerve.get_box2d_instance().fixtures:
                fixture.shape.draw(swerve.get_box2d_instance(), fixture)
        '''for lidar in self.distance_endpoints:
            to_pygame_end = self.CoordConverter.box2d_to_pygame(lidar)
            to_pygame_start = self.CoordConverter.box2d_to_pygame((self.swerve_instances[0].get_box2d_instance().position.x, self.swerve_instances[0].get_box2d_instance().position.y))
            pygame.draw.line(self.screen, ('red'), (to_pygame_start), to_pygame_end, 1)'''

        # draw temporary points of the raycast, uses coordconverter to convert from box2d to pygame
        #print(f'points: {self.raycast_points}')
        for point in self.raycast_points:
            pygame.draw.circle(self.screen, ('red'), self.CoordConverter.box2d_to_pygame(point), 3)
