    ...
    def update_visualization(self):
        ...
        if self.shm_output:
            ...
            if slam_pose and slam_map is not None:
                ...
                orientation_rad = math.radians(slam_pose[2])  # Convert degrees to radians
...
def visualization_main(shm_output_name='slam_output', shm_output_size=16060):
    ...
    # Retry until 'slam_output' shared memory is available
    while True:
        try:
            shm_output = shared_memory.SharedMemory(name=shm_output_name)
            logging.info(f"Visualization connected to shared memory: {shm_output.name}")
            break
        except FileNotFoundError:
            logging.warning(f"Visualization: Waiting for shared memory '{shm_output_name}' to be available...")
    ...
    try:
        ...
        app.close()
        root.destroy()
        logging.info("Visualization process terminated.")
